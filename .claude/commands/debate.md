...
- **Primary is a participant every round, not just Round 1:** The Coordinator (Primary/host model) must produce its own fresh, independently-reasoned rebuttal every round — current position, whether it changed and why, strongest counter-argument, and what would change its mind — before reading peers' this-round responses. A round where the Coordinator only relays or synthesizes peer output without recording and updating its own position is invalid. The Debate Trace and Decision Ledger must show the Primary's position moving (or explicitly holding) round over round, exactly like a peer's.
...
9. On every round from Round 2 onward, the Coordinator itself must produce and freeze its own fresh rebuttal (per the Round-Disclosure Guard's "Primary is a participant every round" rule) before dispatching or reading peers' this-round responses. Do not skip this and go straight to building/dispatching the peer packet — a debate round without a Coordinator position update is invalid.
10. If the user asked to add an in-host subagent (Addition case in the Debate Routing Guard), dispatch it alongside — never instead of — the external peers, under the same guards, and disclose it in the Peer Dispatch Brief.
11. Then follow `<AI_DEV_SHOP_ROOT>/framework/slash-commands/consensus.md` exactly as if the user had invoked `/consensus debate ...`.
