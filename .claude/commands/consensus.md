...
4. If any peer model is inferred rather than explicitly pinned for this run, or the exact resolved model ID cannot be proven, stop before dispatch and print a confirmation gate:
   ...
   - A valid Claude proof is either an exact environment cache hit from `<ADS_MEMORY_ROOT>/.local-artifacts/swarm-consensus/smoke-tests/last-known-good.json` with a real artifact path, or a fresh discovery run that writes a new artifact.
