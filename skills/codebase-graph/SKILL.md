...
- Symlink at `<TARGET_REPO>/graphify-out/` → project knowledge location (so graphify CLI writes transparently)
...
`<TARGET_REPO>/graphify-out/.ads-graphify-status.json` and includes:
...
When stale, prefer `graphify update <TARGET_REPO> --force` for code questions.
Ask for human approval before escalating to semantic extraction.
...
```bash
graphify query "<question>" --graph <TARGET_REPO>/graphify-out/graph.json
graphify path "<A>" "<B>" --graph <TARGET_REPO>/graphify-out/graph.json
graphify explain "<concept>" --graph <TARGET_REPO>/graphify-out/graph.json
```
