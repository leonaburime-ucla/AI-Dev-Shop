...
Load from `AI-Dev-Shop/codebase-analysis/`:
- The relevant `ANALYSIS-*.md` report(s) — all parts if split
- Also load `AI-Dev-Shop/skills/architecture-decisions/SKILL.md` for pattern selection
- Also load `AI-Dev-Shop/skills/design-patterns/SKILL.md` for the target pattern's implementation details
...
Validate against system drivers using `AI-Dev-Shop/skills/architecture-decisions/SKILL.md`. The target must match the actual complexity and team context — not just fix the flaws.
...
Save to `AI-Dev-Shop/codebase-analysis/MIGRATION-<analysis-id>-<YYYY-MM-DD>.md`.
