...
- `<AI_DEV_SHOP_ROOT>/skills/e2e-test-architecture/SKILL.md` — load when the architecture must define system-level testing boundaries, E2E test topology, or test infrastructure that constrains service shape
- `<AI_DEV_SHOP_ROOT>/skills/frontend-accessibility/SKILL.md` — load when the frontend has accessibility compliance requirements (WCAG, ADA, Section 508) that constrain component architecture, rendering strategy, or interaction patterns
- `<AI_DEV_SHOP_ROOT>/skills/postgresql/SKILL.md` — load when making data-layer architecture decisions for PostgreSQL-backed systems (schema strategy, indexing approach, partitioning, connection topology)
