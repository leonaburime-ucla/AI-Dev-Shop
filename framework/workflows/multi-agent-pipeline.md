...
- Software Architect conditionally produces `critical-internal-constraints.md` (designated complex units only; default do-not-produce) or records NOT TRIGGERED before task generation
- Coordinator generates `tasks.md` from the approved ADR plus the combined Design Readiness check (implementation-outline + critical-internal-constraints) before TDD dispatch — not an agent step
...
4. `<ADS_MEMORY_ROOT>/reports/pipeline/<NNN>-<feature-name>/critical-internal-constraints.md` (only if any unit passes the four-part designation test in `<AI_DEV_SHOP_ROOT>/skills/critical-internal-constraints/SKILL.md`) — using `<AI_DEV_SHOP_ROOT>/framework/templates/critical-internal-constraints-template.md`: Binding constraints only, escalation markers where warranted, declared verification surfaces, Required Ordering Constraints instead of call flows, verified source sync. If no unit is designated, do not create the file — write `Critical Internal Constraints: NOT TRIGGERED - <candidate units checked; checked surfaces; why no trigger applies>` to the ADR and Software Architect handoff.
...
- Before generating `tasks.md`, verify Implementation Outline readiness using `<AI_DEV_SHOP_ROOT>/skills/implementation-outline/SKILL.md`: if a trigger applied, `implementation-outline.md` exists with `Status: PRODUCED`; if no trigger applied, Software Architect recorded `Implementation Outline: SKIP - <reason and triggers checked>`. If neither is present, route back to Software Architect.
- As part of the same combined Design Readiness check, verify Critical Internal Constraints readiness using `<AI_DEV_SHOP_ROOT>/skills/critical-internal-constraints/SKILL.md`: `critical-internal-constraints.md` exists with `Status: PRODUCED` and verified source sync (all referenced C-xxx/INV-xxx/ADR/spec IDs exist; artifact not `STALE`), or the ADR/handoff records `Critical Internal Constraints: NOT TRIGGERED - <candidate units checked; checked surfaces; why no trigger applies>`. If neither is present or the artifact is STALE, route back to Software Architect.
- If `critical-internal-constraints.md` exists, record its path in `tasks.md` and reference designated Unit IDs (U-xxx) on tasks that touch them; if not triggered, record the exact NOT TRIGGERED line.
...
- Critical Internal Constraints (`<ADS_MEMORY_ROOT>/reports/pipeline/<NNN>-<feature-name>/critical-internal-constraints.md`) if present, or the recorded NOT TRIGGERED line from `tasks.md`
...
- `WARNING` = likely architectural violations, Implementation Outline deviations, or unrecorded Binding-constraint deviations remain; Coordinator surfaces them (plus any recorded `[CIC_DEVIATION]` entries, unratified `[CIC_PROPOSED]` items, and unresolved `[CIC_REQUESTED]` reports) to the human and asks whether to route back to Programmer or continue downstream. A deviation from an `ESCALATE_SECURITY`/`ESCALATE_IRREVERSIBLE` constraint without a matching `[CIC_DEVIATION_APPROVED]` record is a BLOCKER, not a WARNING
...
| Architecture sign-off | Before tasks.md generation and TDD dispatch — requires clean Constitution Check plus the combined Design Readiness check (outline + critical internal constraints) | Is this ADR acceptable? Are all constitution exceptions justified? Are the outline PRODUCED/SKIP and constraints PRODUCED/NOT TRIGGERED decisions acceptable — including the content of designated-unit Binding constraints, not just their existence? |
