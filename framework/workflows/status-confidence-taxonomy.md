...
| Red-Team finding status | `BLOCKING`, `ADVISORY`, `CONSTITUTION_FLAG` | Adversarial spec finding class | `BLOCKING` blocks Architect; `CONSTITUTION_FLAG` needs human decision; `ADVISORY` travels to ADR context |
...
| Reverse-spec coverage status | `fully_specified`, `partially_specified`, `unspecified` | Extraction completeness by entrypoint | `partially_specified`/`unspecified` critical paths block Architect unless human-approved |
...
- Do not convert Red-Team `ADVISORY` into an ADR waiver. The Architect must
