...
Upgrade `<ADS_PROJECT_KNOWLEDGE_ROOT>/memory/project_memory.md` and `<ADS_PROJECT_KNOWLEDGE_ROOT>/memory/learnings.md` from free-form notes to schema-driven entries that the Observer writes to and reads from in a consistent format. See `<AI_DEV_SHOP_ROOT>/project-knowledge/memory/memory-schema.md`.
