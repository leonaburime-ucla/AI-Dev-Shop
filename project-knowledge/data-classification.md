...
Defines how agents treat sensitive data encountered during pipeline execution. All agents are bound by this policy. Violations are security concerns and are logged to memory-store.md.

Supplements `AI-Dev-Shop/project-knowledge/tool-permission-policy.md`, which governs tool scope. This document governs data handling within those scopes.
...
- **Include in memory-store only when relevant to future pipeline decisions.**
