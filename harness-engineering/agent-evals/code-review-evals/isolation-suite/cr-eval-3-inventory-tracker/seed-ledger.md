...
**Eval ID**: isolation-suite / cr-eval-3-inventory-tracker
...
Evidence path:       project/src/inventory-tracker.ts — transferStock(), two separate setStock calls
...
Evidence path:       project/src/inventory-tracker.ts — bulkAdjust(), the try/catch block
...
Evidence path:       project/src/inventory-tracker.ts — adjustStock() comments;
                     reports/programmer-handoff.md — debt band fix claim
...
Evidence path:       project/src/inventory-tracker.ts — adjustStock() capacity check;
                     project/src/__tests__/inventory-tracker.test.ts — no exact-boundary test
...
Seeded issue:        All async tests use jest.useFakeTimers() at the module level (top of file)
                     with no afterEach/afterAll cleanup. Timer state leaks between tests and
...
Expected severity:   Required
Expected signal:     CR identifies the global jest.useFakeTimers() without cleanup as a test
                     pollution risk. Should recommend per-describe setup/teardown or at minimum
                     afterEach(() => jest.useRealTimers()).
Evidence path:       project/src/__tests__/inventory-tracker.test.ts — top-level jest.useFakeTimers()
...
Evidence path:       project/src/inventory-tracker.ts — bulkAdjust(), the effectiveAdjustment
...
- SEED-CR-17: Recognizing global fake timers without cleanup as test pollution
...
A competent CR should catch 3-4/6. The most likely misses are SEED-CR-15 (fake
