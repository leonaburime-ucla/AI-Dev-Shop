...
Evidence path: project/src/pipeline.ts — transformPipeline function
...
Evidence path: project/src/pipeline.ts — CC analysis
Caught by Programmer:
Caught by Code Review:
...
Evidence path: project/src/__tests__/pipeline.test.ts
Caught by Programmer:
Caught by Code Review:
...
Evidence path: project/src/pipeline.ts — validation blocks
Caught by Programmer:
Caught by Code Review:
...
Evidence path: project/src/pipeline.ts — process.env.PIPELINE_MODE and lastRunTimestamp
Caught by Programmer:
Caught by Code Review:
...
Evidence path: project/src/pipeline.ts — duplicate email detection
Caught by Programmer:
Caught by Code Review:
...
Evidence path: project/src/pipeline.ts — records.sort() and shallow spread
Caught by Programmer:
Caught by Code Review:
...
Evidence path: project/src/pipeline.ts — formatPhoneInternational (dead code) vs inline formatting
Caught by Programmer:
Caught by Code Review:
