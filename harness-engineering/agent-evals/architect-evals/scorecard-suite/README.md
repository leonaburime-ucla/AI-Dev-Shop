...
python3 harness-engineering/quality/scripts/prepare_eval_run.py \
  harness-engineering/agent-evals/architect-evals/scorecard-suite \
  run-001
```
...
- The canonical `run-manifest.tsv` and `run-results.tsv` were reset after the
  suite changed. A fresh Architect run is required before using this revision's
  scores.
