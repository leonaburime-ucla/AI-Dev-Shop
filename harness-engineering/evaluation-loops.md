...
Use `harness-engineering/self-validation.md` for the implementer's own bounded runbook.
...
When models, hosts, or task shape improve enough that the extra judge no longer adds clear lift, re-test the pattern with `harness-engineering/load-bearing-harness-audit.md` instead of keeping it by habit.
