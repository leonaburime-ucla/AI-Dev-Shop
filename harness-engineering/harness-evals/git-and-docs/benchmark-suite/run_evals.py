#!/usr/bin/env python3
"""Run git-strategy and code-documentation evals."""
...
GRADING_CRITERIA: dict[str, dict[str, list[str]]] = {
    "SEED-GD-01": {
        "pass_signals": ["feature/305", "branch", "tdd", "pipeline-state", "create"],
        "fail_signals": ["work on main", "skip branch", "after implementation"],
    },
    "SEED-GD-02": {
        "pass_signals": ["spec hash", "abc123", "adr", "test certification", "security"],
        "fail_signals": ["auto-merge", "push automatically", "skip pr"],
    },
    "SEED-GD-03": {
        "pass_signals": ["document", "retry", "timeout", "side effect", "db write", "stripe", "jsdoc"],
        "fail_signals": ["skip doc", "no documentation needed", "obvious"],
    },
    "SEED-GD-04": {
        "pass_signals": ["no doc", "skip", "obvious", "trivial", "intentionally", "undocumented", "one-liner"],
        "fail_signals": ["add jsdoc", "add comment", "must document", "formats cents"],
    },
    "SEED-GD-05": {
        "pass_signals": ["required", "side effect", "db", "queue", "publish", "missing doc", "flag"],
        "fail_signals": ["pass", "no finding", "looks good", "approved without"],
    },
    "SEED-GD-06": {
        "pass_signals": ["recommended", "noise", "restate", "remove", "bloat", "not required"],
        "fail_signals": ["required finding", "hard block", "must fix before"],
    },
}
...
Based on the rules above, describe exactly what you do. Be specific. 150 words max."""
